...
def req_dining_menu():
    ...
    if hour < 0:
        ...
        if len(result_list) == 0:
            ...
            return make_json_error("Could not find any menus for "
                                   "{0}/{1}/{2}.".format(month, day, year))
        return jsonify(num_results=len(result_list), menus=result_list)

    # hour argument was supplied (or implied to be now), so find a menu for
    # that datetime
    ...
    if not result:
        return make_json_error(
            "No menu found for {0} at {1:02d}:{2:02d} {3}/{4}/{5}.".format(
                eatery, int(hour), int(minute), month, day, year))
...
def req_dining_hours():
    ...
    # Find the hours document(s) for this eatery on this date.
    # Exclude the ObjectID from the result.
    # NOTE: some eateries open/close multiple times a day -> thus, multiple
    # hours documents per day
    results = hours.find(
        {'eatery': eatery,
         'year': year,
         'month': month,
...
def req_dining_find():
    ...
    if len(result_list) == 0:
        ...
        return make_json_error("Could not find {0} in any of the eatery "
                               "menus.".format(food))
...
def req_dining_nutrition():
    ...
    if not result:
        return make_json_error("No nutritional information available for "
                               "{0}.".format(food))
...
def req_dining_open():
    ...
    # if no hour provided, return all open eateries on provided (or implied)
    # date
    if hour < 0:
        results = hours.find(
            {'year': year,
        ...
        if len(open_eateries) == 0:
            ...
            return make_json_error("No open eateries on {0}/{1}/{2}.".format(
                month, day, year))
    ...
    if len(open_eateries) == 0:
        # no open eateries found
        return make_json_error(
            "No open eateries at {0:02d}:{1:02d} on {2}/{3}/{4}.".format(
                int(hour), int(minute), month, day, year))
    return jsonify(open_eateries=open_eateries)
...
def req_dining_all_food():
    ...
    if not eatery:
        return make_json_error("No eatery name provided or invalid "
                               "eatery name.")
    ...
    if not result:
        return make_json_error(
            "No food information available for {0}.".format(eatery))
    return jsonify(**result)
...
def get_dining_date():
    ''' Return the current dining date as a date object.
        This extends until 2am the next morning.
        (e.g. 1:50AM on 11/17/2016 is reset to 11/16/2016)
    '''
    today = date.today()
    now = datetime.now()
    if now.hour < 2:
...
def get_dining_datetime():
    ''' Return the current dining date as a datetime object.
        This extends until 2am the next morning. If it happens to be
        in the awkward 12am - 2am period, reset to 11:59 the previous day
        for convenience.
    '''
    now = datetime.now()
    if now.hour < 2:
